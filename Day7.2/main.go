...
func hasValidValues(c Calculation) bool {
	...
	// divide total by first value and recurse
	if hasValidValues(Calculation{c.total, append([]int{c.values[0] * c.values[1]}, c.values[2:]...)}) {
		return true
	}

	// add first two values and recurse
	if hasValidValues(Calculation{c.total, append([]int{c.values[0] + c.values[1]}, c.values[2:]...)}) {
		return true
	}

	// concatenate first two values and recurse
	newValueStr := fmt.Sprintf("%d%d", c.values[0], c.values[1])
	newValue, _ := strconv.Atoi(newValueStr)
	// replace first two values with new value and recurse
	return hasValidValues(Calculation{c.total, append([]int{newValue}, c.values[2:]...)})
}
